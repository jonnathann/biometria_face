function [eer, accept] = far_ffr(label, dist, yy)
%l = load('example.mat');
%[eer, accept] = ts_z_eer_aux(l.label, l.dist, l.yy);
[eer, accept] = ts_z_eer_aux(label, dist, yy);
end

function [eer, accept] = ts_z_eer_aux(label, dist, yy)
...
[accuracy, erro, reje] = accuracy_threshold(predicts_all, threshold);
end
...
for i = 1:size(predicts_all)
    if predicts_all(i, 1) > threshold && predicts_all(i, 2) == predicts(i, 3)
    ...
    accuracy = (accept*100)/size(predicts_all);
    erro = (err * 100)/size(predicts_all);
    reje = (rejected * 100)/size(predicts_all);
end
end
