function get_table_results(folder, classify, extraction, param_extractor)
    ...
    for i = 3:size(directory_names, 2)
        ...
        if strcmp(classify{1}, 'ENSEMBLE-ELM') && strcmp(extraction, 'LOCAL-BINARY-PATTERN')
            accuracy = {aux.structureEsembleELM.mean_accuracy};
            standard = {aux.structureEsembleELM.std};
            param_classify = {aux.structureEsembleELM.components};
            param_extraction = {aux.structureEsembleELM.mask};
            fprintf(fid, strcat('\t\t', num2str(param_classify{1}), '\t\t',num2str(accuracy{1}), '\t\t', num2str(standard{1}), '\t\t' ,param_extraction{1}, '\n\n'));
            fprintf(fid, strcat('\t', '-----------------------------------------------------------------------' ,'\t\t\n\n'));
