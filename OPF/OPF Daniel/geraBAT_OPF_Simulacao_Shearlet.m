...
folds=10;
arq='C:\Users\Daniel\Desktop\OPF\OPF\ExecutaTestesOPF_Shearlet.bat';
fid=fopen(arq,'w');
for c=2:length(Car)
    for d=1:length(Distancias)
        for w=1:3
            for k=1:folds
                ArqTreino=strcat(Car{c},'\',Car{c},'_Shearlet_Nivel',int2str(4-w),'_Fold_',int2str(k),'_Treino.opf');
                ArqTeste=strcat(Car{c},'\',Car{c},'_Shearlet_Nivel',int2str(4-w),'_Fold_',int2str(k),'_Teste.opf');
                ArqDist=strcat('Distancias\',Car{c},'_Shearlet_Nivel',int2str(4-w),'_',Distancias{d},'.dat');
...
%         for w=1:1
%             for k=1:10
%                 ArqTreino=strcat('FaceIris\FaceIris_',Abordagens{a},'_Shearlet_Nivel',int2str(4-w),'_Fold_',int2str(k),'_Treino.opf');
%                 ArqTeste=strcat('FaceIris\FaceIris_',Abordagens{a},'_Shearlet_Nivel',int2str(4-w),'_Fold_',int2str(k),'_Teste.opf');
%                 ArqDist=strcat('Distancias\FaceIris_',Abordagens{a},'_Shearlet_Nivel',int2str(4-w),'_',Distancias{d},'.dat');
%                 fprintf(fid,'opf_train %s %s \n',ArqTreino,ArqDist);
%                 fprintf(fid,'opf_classify %s %s \n',ArqTeste,ArqDist);
%                 fprintf(fid,'opf_accuracy %s \n',ArqTeste);
%                 ArqOut=strcat('C:\Users\Daniel\Desktop\OPF\OPF\',ArqTreino,'.out');
