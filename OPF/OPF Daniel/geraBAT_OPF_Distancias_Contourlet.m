...
arq='D:\OPF\OPF\GeraArquivoDistanciaContourlet_Multi.bat';
  ...
  for c=1:length(Car)
      for w=1:length(Wavelets)
          for d=1:length(DistOP)
              ArqEntrada=strcat('Completos\',Car{c},'_Contourlet_',Wavelets{w},'.opf');
              fprintf(fid,'opf_distance %s %i 1 \n',ArqEntrada,DistOP(d));
              ArqDist=strcat(Car{c},'_Contourlet_',Wavelets{w},'_',Distancias{DistOP(d)},'.dat');
...
for a=1:length(Abordagens)
    for w=1:length(Wavelets)
        for d=1:length(DistOP)
            ArqEntrada=strcat('Completos\FaceIris_',Abordagens{a},'_Contourlet_',Wavelets{w},'.opf');
            fprintf(fid,'opf_distance %s %i 1 \n',ArqEntrada,DistOP(d));
            ArqDist=strcat('FaceIris_',Abordagens{a},'_Contourlet_',Wavelets{w},'_',Distancias{DistOP(d)},'.dat');
            fprintf(fid,'rename distances.dat %s \n',ArqDist);
            fprintf(fid,'move %s Distancias \n',ArqDist);
        end
    end
