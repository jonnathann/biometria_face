...
for Tec=1:length(TecRed)
    for car=1:length(Caracteristicas)
        arq=strcat('C:\Users\Daniel\Documents\Documentos\Mestrado - Backup\SDUMLA\Bases Construidas\',...
            Caracteristicas{car},'\Curvelet\RedDim\',TecRed{Tec},...
            '\',Caracteristicas{car},'_Curvelet_',TecRed{Tec},'.pessoa1.mat')
        load(arq);
        QtdCar=size(BaseDados.X,2);
        Partes=ceil(QtdCar/QtdParte);
        clear BaseDados
        for parte=Partes:Partes
            Dados=[];
            for p=1:pessoas
                load(strcat('C:\Users\Daniel\Documents\Documentos\Mestrado - Backup\SDUMLA\Bases Construidas\',...
                    Caracteristicas{car},'\Curvelet\RedDim\',TecRed{Tec},...
            '\',Caracteristicas{car},'_Curvelet_',TecRed{Tec},'.pessoa',int2str(p),'.mat'));
                if parte<Partes
