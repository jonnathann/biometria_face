...
function_extraction = {'db2', 'db4', 'sym3', 'sym4', 'sym5'};
%function_extraction = {'40', '43', '46', '49', '52'};
%function_extraction = {'16x16', '20x20', '24x24', '28x28'};
%datasets = {'YALE_Viola_Jones-FOURIER-TRANSFORM'};
datasets = {'YALE_Viola_Jones-WAVELET-TRANSFORM-LEVEL:3-COMBINATION:LL'};
%datasets = {'YALE_Viola_Jones-LOCAL-BINARY-PATTERN'};

for dataset = 1:size(datasets, 2)
    
    for fct = 1:size(function_extraction, 2)
        
        name_dataset = strcat(datasets{dataset}, '/image_feature_', num2str(fct));
        ...
        gamma = [2e-3, 2e-4, 2e-5, 2e-6, 2e-7, 2e-8, 2e-9];
        ...
        for gamm = 1:size(gamma, 2)
            ...
            structureSVM = struct('Dataset', datasets{dataset}, 'fwavelet', function_extraction{fct}, 'Gamma', num2str(gamma(gamm)),'mean_accuracy', mean(accuracies), 'std', std(accuracies), 'Erro', mean(errors),'samples', accuracies);
            result = strcat('Result-SVM-Fwavelet-', function_extraction{fct},'-gamma-', num2str(gamma(gamm)), datasets{dataset}); 
            save(strcat(result,'.mat'), 'structureSVM');
            result_gamma(gamm) = mean(errors);
        end
        subplot(4, 3, fct);
        plot(result_gamma);
        title(function_extraction{fct});
        fprintf('\nFinish: %i', fct);
        pause(5);
    end
end    
