...
def cross_validation():
    ...
    for conv_pool in range(len(nb_conv_pool)):
        for nh in range(len(nb_neurons)):
            ...
            #str_name_directory ='/home/gria/Área de Trabalho/CNN dissertação final/AlexNet/RESULT-AlexNet-DATASET:'+str(database[0])
            
            str_name_directory = '/media/clodoaldo/Dados/Jonnathann/Experimentos Novos/Deep Learning/Convolutional Neural Networks/AlexNet/RESULT-CNN-DATASET:'+str(database[0])
            ...
            str_name_file = 'RESULT-AlexNet-CONV-DIMENSION:'+str(nb_conv_pool[conv_pool][0])+'-POOL DIMENSION:'+str(nb_conv_pool[conv_pool][1])+'-NEURONS-HIDDEN-LAYER:'+str(nb_neurons[nh])+'.txt'
            ...
            for i, (id_train, id_test) in enumerate(folds):
                ...
                print "\nFold:", i+1, " | AlexNet | conv dimension:(", nb_conv_pool[conv_pool][0],'x',nb_conv_pool[conv_pool][0] ,")-pool dimension:(", nb_conv_pool[conv_pool][1],'x',nb_conv_pool[conv_pool][1],")-Neurons hidden layer:", nb_neurons[nh], "\n\n"
                ...
                #salva a acurácia de cada execução
