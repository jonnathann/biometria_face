...
import os as drt
import shutil as sh
import pre_processing_atc as dt




def autoencoder(input_dim, data, neurons_layer, neurons_hidden_layer, epoch, batch):
    
    input_img = Input(shape=(input_dim,))
    #encoded layer 1
    encoded = Dense(neurons_layer, activation='relu')(input_img)
    #encoded layer 2
    encoded = Dense(neurons_hidden_layer, activation='relu')(encoded)#Features
    #decoded layer 3
...
def feature_extraction():
    ...
    for nhl in range(len(neurons_hidden_layer)):
        ...
        encoded_images, _ = autoencoder(input_dim, data, neurons_layer, neurons_hidden_layer[nhl], epochs, batch_size)
    
    
        #New directory
        str_name_directory ='/media/clodoaldo/Dados/Jonnathann/Experimentos Novos/Deep Learning/Autoencoders/Autoencoders Features Extraction/Autoencoder-'+str(database[0])
        #str_name_directory = "/media/clodoaldo/Dados/Jonnathann/Experimentos Novos/Autoencoder/Autoencoders Features Extraction/Autoencoder-"+str(database[0])
        if drt.path.exists(str_name_directory) == False:
                
            drt.mkdir(str_name_directory)
        
        #File name
        str_name_file = 'AUTOENCODER-'+str(database[0])+'-NEURONS-HIDDEN-LAYER:'+str(neurons_hidden_layer[nhl])+'.txt'
