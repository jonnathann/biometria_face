function k_fold()
    ...
    for all_parameter = 1:size(all_parameters, 1)        
        ...
        for i = 1:nfold
            ...
            str = strcat('Fold=', num2str(i), '-Waveltet=', fwavelets{variables_parameters(2)}, '-Neurons=', num2str(neurons(variables_parameters(3))), '-Dataset=', datasets{variables_parameters(1)});
            disp(str);
            
            
        end
        structureELM = struct('Dataset', datasets{variables_parameters(1)}, 'Fwavelet', fwavelets{variables_parameters(2)}, 'Neurons', num2str(neurons(variables_parameters(3))),'mean_accuracy', mean(accuracies), 'std', std(accuracies), 'samples', accuracies);
        result = strcat('Result-ELM-Fwavelet-', fwavelets{variables_parameters(2)},'-Neurons-', num2str(neurons(variables_parameters(3))), datasets{variables_parameters(1)}); 
