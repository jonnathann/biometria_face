function processing_table_results()
    ...
    datasets = {'YALE', 'JAFFE', 'ORL', 'AR', 'SDUMLA'};
    extraction = 'Autoencoder-Sparse';
    param_extractor = 'DIMENSION';
    classifies = {'ELM', 'NEURONS'};
    ...
    %talebas para os autoencoders datasets: YALE, AR e SDUMLA
    
    if strcmp(extraction, 'Autoencoder')
        
        for dts = 1:size(datasets, 2)
            
            str_folder = strcat('RESULTS-',extraction, '-',datasets{dts});
            disp(strcat('TABLE-',str_folder));
            get_table_results(str_folder, classifies, extraction, param_extractor);
        end 
    end  
    
    if strcmp(extraction, 'Autoencoder-Sparse')
